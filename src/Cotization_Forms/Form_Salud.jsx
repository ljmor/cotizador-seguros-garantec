...
const ComparisonStep = ({ onSelectPlan }) => {
    ...
    useEffect(() => {
        const fetchPlanes = async () => {
            try {
                const respuesta = await fetch('http://localhost:5000/api/planes?tipo=Salud');
...
const InsuranceTypeCard = ({ icon, label, selected, onClick }) => (
    <motion.div whileHover={{ y: -5 }} onClick={onClick} className={`relative text-center p-4 border rounded-xl cursor-pointer transition-all duration-300 ${selected ? 'bg-white border-teal-400 shadow-lg scale-105' : 'bg-gray-50 border-gray-200 hover:bg-white hover:shadow-md'}`}>
...
export default function Form_Salud({ navigateTo, cotizacionIdParaEditar = null }) {
    ...
    const handleDependentsChange = (index, event) => { const nuevosDependientes = formData.dependientes ? [...formData.dependientes] : []; nuevosDependientes[index][event.target.name] = event.target.value; setFormData(prevState => ({ ...prevState, dependientes: nuevosDependientes })); };
    const addFamilyMember = () => { const nuevosDependientes = formData.dependientes ? [...formData.dependientes] : []; setFormData(prevState => ({ ...prevState, dependientes: [...nuevosDependientes, { id: Date.now(), genero: '', relacion: '', edad: '' }] })); };
    ...
    const validarCampos = (pasoActual) => {
        const camposRequeridosPaso1 = { nombres: 'Nombres completos', apellidos: 'Apellidos completos', telefono: 'Teléfono', email: 'E-mail', provincia: 'Provincia', canton: 'Cantón / Ciudad', callePrincipal: 'Calle Principal' };
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 'form2' && (
                            <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" exit="exit" transition={{ duration: 0.3 }}>
                                <DependentsStep onQuote={handleQuote} formData={formData} handleChange={handleChange} handleDependentsChange={handleDependentsChange} addFamilyMember={addFamilyMember} />
                            </motion.div>
                        )}
                        {step === 'comparison' && (
