        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormInput label="Fecha de nacimiento" type="date" defaultValue="2000-05-06" />
             <div>
                ...
                <select className="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
            ...
            <div className="space-y-6 mt-6">
                 <div>
                    ...
                    <div className="flex gap-2">
                        <button className="flex-1 py-2 px-4 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 transition">Beneficios</button>
                        <button className="flex-1 py-2 px-4 rounded-lg bg-indigo-100 text-indigo-700 border border-indigo-200 transition">Respaldo de la aseguradora</button>
                        <button className="flex-1 py-2 px-4 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 transition">Precio</button>
                    </div>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-600 mb-2">¿Es una cotización familiar?</label>
                    <div className="flex gap-2">
                        <button onClick={() => setIsFamily(true)} className={`py-2 px-6 rounded-lg border transition ${isFamily ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300'}`}>Sí</button>
                        <button onClick={() => setIsFamily(false)} className={`py-2 px-6 rounded-lg border transition ${!isFamily ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300'}`}>No</button>
                    </div>
                </div>
                {isFamily && (
                    <div>
                        ...
                        {familyMembers.map((member, index) => (
                            <div key={member.id} className="flex items-center gap-2 mb-2">
                                <FormInput placeholder="Genero" />
                                <FormInput placeholder="Relacion" />
                                <FormInput placeholder="Edad" />
...
export default function Form_Salud({ navigateTo }) {
    const [step, setStep] = useState('form1'); // Pasos: form1, form2, comparison
    ...
    const handleBack = () => {
        ...
        else if (step === 'form1') navigateTo('landing'); // Asume que 'navigateTo' es una prop para cambiar de página
    };
        ...
        <div className="bg-white font-sans w-full h-full flex flex-col min-h-screen">
            <div className="flex flex-col lg:flex-row flex-grow">
                ...
                <main className="w-full lg:w-2/3 bg-slate-50 p-6 sm:p-8 md:p-12 overflow-y-auto">
                    ...
                    {step === 'form2' && <DependentsStep onQuote={handleQuote} />}
