...
const ProfileStep = ({ onQuote, formData, handleChange }) => (
    <>
        ...
        <div className="space-y-6 mt-6">
            <div>
                <FormSelect required label="Ocupación" name="ocupacion" value={formData.ocupacion || ''} onChange={handleChange}>
                    <option value="">Elija la profesion del asegurado</option><option>Ingeniero/a</option><option>Doctor/a</option><option>Abogado/a</option>
                </FormSelect>
            </div>
        </div>
        <div className="flex justify-end mt-8"><motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onQuote} className="px-8 py-3 rounded-lg bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition shadow-lg">Cotizar</motion.button></div>
    </>
);
...
const ComparisonStep = ({ onSelectPlan }) => {
    ...
    useEffect(() => {
        const fetchPlanes = async () => {
            try {
                const respuesta = await fetch('http://localhost:5000/api/planes?tipo=Vida');
    ...
    return (
        <div>
            <h1 className="text-3xl md:text-4xl font-bold text-indigo-800">Planes de Vida Disponibles</h1>
            <p className="text-gray-500 mt-1 mb-8">Seleccione la mejor opción de seguro de Vida para ti.</p>
...
export default function Form_Vida({ navigateTo, cotizacionIdParaEditar = null }) {
    ...
    const validarCampos = (pasoActual) => {
        const camposRequeridosPaso1 = { nombres: 'Nombres completos', apellidos: 'Apellidos completos', sexo: 'Sexo', telefono: 'Teléfono', email: 'E-mail', provincia: 'Provincia', canton: 'Cantón / Ciudad', callePrincipal: 'Calle Principal' };
        const camposRequeridosPaso2 = { ocupacion: 'Ocupación' };
                    ...
                    {step !== 'comparison' && (
                        <>
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-10">
                                <InsuranceTypeCard icon={<CancerIcon/>} label="Oncológico" selected={formData.tipoSeguroVida === 'oncologico'} onClick={() => handleChange({ target: { name: 'tipoSeguroVida', value: 'oncologico', type: 'button' }})} />
                                <InsuranceTypeCard icon={<LifeIcon/>} label="Vida" selected={formData.tipoSeguroVida === 'vida'} onClick={() => handleChange({ target: { name: 'tipoSeguroVida', value: 'vida', type: 'button' }})} />
                                <InsuranceTypeCard icon={<AccidentIcon/>} label="Accidentes" selected={formData.tipoSeguroVida === 'accidentes'} onClick={() => handleChange({ target: { name: 'tipoSeguroVida', value: 'accidentes', type: 'button' }})} />
                            </div>
                        </>
                    )}
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 'form2' && (
                            <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" exit="exit" transition={{ duration: 0.3 }}>
                                <ProfileStep onQuote={handleQuote} formData={formData} handleChange={handleChange} />
                            </motion.div>
                        )}
                        {step === 'comparison' && (
                ...
                <aside className="hidden lg:block w-1/3 bg-[#6074F3] p-8 sticky top-0 h-screen">
                    <div className="flex flex-col items-center justify-center h-full">
                        <div className="w-full bg-gray-300 rounded-2xl shadow-2xl overflow-hidden aspect-w-9 aspect-h-16">
                            <img src="/src/assets/img_form_vida.png" alt="Manos unidas sobre una mesa" className="w-full h-full object-cover" />
                        </div>
