...
// Se han importado los iconos para reemplazar los SVGs
import { GiDeadHead, GiPlagueDoctorProfile } from 'react-icons/gi';
import { TbRibbonHealth } from 'react-icons/tb';

// --- Iconos SVG restantes ---
// Se mantienen los que aún son necesarios
const BackArrowIcon = () => (
...
const PlusCircleIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-indigo-600 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
    </svg>
);

// --- Componentes para los nuevos iconos ---
// Ahora usamos los componentes de react-icons con sus estilos correspondientes
const CancerIcon = () => (
    ...
    return (
        <>
            ...
            <div className="space-y-6 mt-6">
                ...
                {isFamily && (<div><label className="block text-sm font-medium text-gray-600 mb-2">Informacion Familiar</label>{familyMembers.map((member, index) => (<div key={member.id} className="flex items-center gap-2 mb-2"><FormInput placeholder="Genero" /><FormInput placeholder="Relacion" /><FormInput placeholder="Edad" />{index === familyMembers.length - 1 && <PlusCircleIcon onClick={addFamilyMember} />}</div>))}</div>)}
            </div>
...
export default function Form_Vida({ navigateTo }) {
    ...
    // Función de navegación hacia atrás actualizada.
    const handleBack = () => {
        if (step === 'comparison') setStep('form1');
        else if (step === 'form2') setStep('form1');
        else if (step === 'form1') navigateTo('landing'); // Vuelve al Landing
    };
    
    const progress = step === 'form1' ? 33 : step === 'form2' ? 66 : 100;
        ...
        <div className="bg-white font-sans w-full h-full flex flex-col">
            <div className="flex flex-col lg:flex-row flex-grow">
                ...
                <main className="w-full lg:w-2/3 bg-slate-50 p-6 sm:p-8 md:p-12 overflow-y-auto">
                    ...
                    {step !== 'comparison' && (
                         <>
                            <div className="w-full bg-gray-200 rounded-full h-2 mb-8"><div className="bg-teal-400 h-2 rounded-full transition-all duration-500" style={{ width: `${progress}%` }}></div></div>
                            <h1 className="text-3xl md:text-4xl font-bold text-indigo-800">Cotizador de Vida</h1>
